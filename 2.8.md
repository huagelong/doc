...
```
    ...
    Available commands:
     ...
     job
      ...
      job:reload      reload the job server
     ...
     server
      server:reload   reload all server
...
```
php trensy server:restart -d
```
 命令后面加"-d"表示命令在后台执行
 "server,httpd,job,rpc"命令组里面的"start,restart"有效